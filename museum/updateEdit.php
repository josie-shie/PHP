...
if( $_FILES["musImg"]["error"] === 0 ) {
    ...
    // 建立完整名稱
    $musImg = $strDatetime.".".$extension;

    // 若上傳成功，則將上傳檔案從暫存資料夾，移動到指定的資料夾或路徑
    if( move_uploaded_file($_FILES["musImg"]["tmp_name"], "./images/".$musImg) ) {
        /**
         * 刪除先前的舊檔案: 
        ...
        $sqlGetImg = "SELECT `musImg` FROM `museum` WHERE `id` = ? ";
        ...
        if($stmtGetImg->rowCount() > 0) {
            ...
            if($arrImg['musImg'] !== NULL){
                // 刪除實體檔案
                @unlink("./images/".$arrImg['musImg']);
            } 
